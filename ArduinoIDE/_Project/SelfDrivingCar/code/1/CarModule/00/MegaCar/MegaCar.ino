...
#include <Arduino.h>
 ...
 //====插入變數Start====
   ...
   SoftwareSerial ESP32Serial(1,2);
     //4way循跡感測器腳位
...
void loop()
{
  //接收訊息：ESP32→Mega
     ...
     if (str.indexOf(MAP_SET) != -1) {
      servoCarBox.write(ANGLE_CAR_BOX_OPEN);      delay(1000);
      ...
      //儲存收件人陣列(格式:goodsLoad,姓名,商品,倉庫X,倉庫Y,收件人X,收件人Y)
       ...
       servoCarBox.write(ANGLE_CAR_BOX_CLOSE);      delay(1000);
      //A*路徑歸劃
   ...
   //等待AI人臉識別
     ...
     if (aiId == FACE_ID1) {
      servoCarBox.write(ANGLE_CAR_BOX_OPEN);      delay(1000);
      servoAiCam.write(ANGLE_AI_CAM_FRONT);      delay(1000);
    }
    }
  ...
  float percent = getBusPowerPercent();  //取得電池當前電壓百分比(小數)
...
void trackForward() {
  ...
  if ((trackSensor[TRACK_LEFT] == TRIGGLED) && !(trackSensor[TRACK_RIGHT] == TRIGGLED)) {
    diagonalRight();} else if (!(trackSensor[TRACK_LEFT] == TRIGGLED) && (trackSensor[TRACK_RIGHT] == TRIGGLED)) {
    diagonalLeft();} else {
    forward();}
  }  //end while
 }//end trackForward
...
void goCar() {
   ...
   for (int i = 0; i < tokenLen; i++) {
     if (pathCarMotor[i] == "F") {
       ...
       //循跡感測
       //雲端平台模擬GPS
   AiDetect();trackForward();ESP32Serial.print(String(CAR_GPS) + "," + pathXY[countPathXY++]);      isFrontArrive = false;
       delay(FTimer);
       Serial.println("前,");
