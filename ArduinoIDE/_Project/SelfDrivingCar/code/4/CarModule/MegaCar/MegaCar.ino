//Generated Date: Mon, 18 Dec 2023 11:32:25 GMT
//★先不使用測低電壓，改用console.hivemq.cloud的car/lowPower模擬代替
...
void loop() {
  ...
  while (Serial1.available()) {
    //Serial.println("ESP32Serial.available");
    ...
    if (str.indexOf(MAP_SET) != -1) {
      ...
      delay(1000);
      //A*路徑歸劃
      bool isFindPath = starAStarPlan(Recipient_POINT);  //開始導航
      starNav(isFindPath, Recipient_POINT);              //抵達目的地,AI鏡頭朝向,準備人臉識別收貨人
      if (isFindPath) {
        standByAiCam();
      }
      //完成送貨，發送LINE通知收貨人(格式:LINE_NOTIFY,姓名,商品)
      if (isFindPath) {
        Serial1.print(String(LINE_NOTIFY) + ":" + recipient[0] + "," + recipient[1]);
      }
    } else if (str.indexOf(CAR_LOWPOWER) != -1) {
      //★小車重開機再測試
      Serial.println("小車沒電啦！(模擬)");      
      //開始充電
      startCharge();
    }
  }  //end while
  ...
  if (digitalRead(BUTTON_SIGN_PIN) == 1) {
    ...
    signSong();
    //★先不用。A*路徑歸劃回物流站
    //bool isFindPath = starAStarPlan(GOODS_POINT);  //開始導航
    //starNav(isFindPath, GOODS_POINT);              //抵達物流站，車子原地轉向(車頭要朝下D，否則貨斗開啟方向會卡住)
    //carHeadTurn('D');
  }

  //取得電池當前電壓百分比(小數)
  float percent = getBusPowerPercent();
  if (currentPoint == GOODS_POINT && percent <= 0.2) {
    //★先不使用測低電壓，改用console.hivemq.cloud的car/lowPower模擬代替
    //startCharge();
  }
...
//設起訖點
void setStartEndPoint(String startP, String endP) {
  startPoint = startP;
  endPoint = endP;
...
bool starAStarPlan(char* target) {
  Serial.println(currentPoint + "→" + target);
...
float getBusPowerPercent() {
  ...
  float betteryPercent = ((busvoltage - minVolt) / (maxVolt - minVolt));
...
void goCar() {
  ...
  for (int i = 0; i < tokenLen; i++) {
    if (pathCarMotor[i] == "F") {
      ...
      delay(FTimer);
      //雲端平台模擬GPS
      Serial1.print(String(CAR_GPS) + "," + pathXY[countPathXY++]);
      Serial.print("前,");
...
void signSong() {
  ...
  for (int thisNote = 0; thisNote < size; thisNote++) {
    ...
    buzz(BEEP_PIN, melody[thisNote], noteDuration);
    int pauseBetweenNotes = noteDuration * 1.30;
    delay(pauseBetweenNotes);
    // stop
    buzz(BEEP_PIN, 0, noteDuration);
  }
}
//Mario
