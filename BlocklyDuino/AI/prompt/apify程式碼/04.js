    ...
    try {
        const crawler = new PuppeteerCrawler({
            ...
            requestHandler: async ({ page, request, log }) => {
                ...
                // 等待所有需要的元素加載完成
                await page.waitForSelector('.card-subtitle');
                await page.waitForSelector('.carousel-item.caption.active p');
                await page.waitForSelector('.last-time');

                // 抓取特定數值
                const value1 = await page.$eval('.card-subtitle', el => el.innerText); // 抓取 506
                const value2 = await page.$eval('.carousel-item.caption.active p', el => el.innerText); // 抓取 1708.14
                const lastReportTime = await page.$eval('.last-time', el => el.innerText.replace('最後回報時間: ', '').trim()); // 抓取時間並去除前面的文字
                
                // 將結果儲存到 Apify 的 OUTPUT
                const result = {
                    value1,
                    value2,
                    lastReportTime,
                ...
                log.info('Extracted values have been saved to the Output page.');
