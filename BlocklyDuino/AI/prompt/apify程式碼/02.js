    ...
    try {
        ...
        },
        {
            ...
            domain: 'ecloud.ntpc.edu.tw'
                ...
                // 抓取整個頁面內容並儲存到 Apify 的 Output 頁面
                //const pageData = await page.content();
                //await Actor.setValue('OUTPUT', pageData);

				const todayValue = await page.$eval('.greenValue#today', el => el.innerText);
                log.info(`Today Value: ${todayValue}`);
                
				//await Actor.setValue('todayValue', { todayValue });//儲存在key-value store
				await Actor.pushData({ todayValue });//儲存在Dataset
                log.info(todayValue);
            },
        });
