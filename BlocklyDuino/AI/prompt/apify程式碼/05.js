    ...
    try {
        const crawler = new PuppeteerCrawler({
            ...
            requestHandler: async ({ page, request, log }) => {
                ...
                // 導航到受保護的頁面
                await page.goto('https://ecloud.ntpc.edu.tw/ec25001.do');
                
                // 等待目標元素加載完成
                await page.waitForSelector(`#IN10-NT0068-C231-217`, { timeout: 60000 });

                // 選取目標區塊
                const targetBlock = await page.$('#IN10-NT0068-C231-217');
                if (!targetBlock) {
                    log.info('No element with id="IN10-NT0068-C231-217" found.');
                    return;
                }

                // 抓取卡片副標題值（506）
                const cardSubtitleValue = await targetBlock.$eval('.card-subtitle', el => el.innerText);

                // 抓取累計電量（1712.12）
                const carouselValue = await targetBlock.$$eval('.carousel-item.caption', items => {
                    const item = items.find(el => el.getAttribute('data-type') === '501200');
                    return item ? item.querySelector('p').innerText : null;
                });

                // 抓取最後回報時間
                const lastReportTime = await targetBlock.$eval('.last-time', el => 
                    el.innerText.replace('最後回報時間: ', '').trim()
                );

                // 儲存結果
                const result = {
                    cardSubtitle: cardSubtitleValue,
                    carouselValue,
                    lastReportTime,
                };

                //await Actor.setValue('OUTPUT', result);//儲存在key-value store
				await Actor.pushData(result);//儲存在Dataset
                log.info('Filtered values have been saved to the Output page.');
            },
        });

        await crawler.run([{ url: 'https://ecloud.ntpc.edu.tw/ec25001.do' }]);
    } catch (error) {
